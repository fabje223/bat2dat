...
#### Release August, 2025
*reportGenerat0r()* was removed in the updated version and replaced by *renderReport()*.

### renderReport()
#### NEW Function since August, 2025
*renderReport()* will initiate the RMarkdown script in the inst > rmd folder to generate an HTML Report of the data. The function can only read one sample at the time (unless the RMarkdown script is modified).
...
For custom RMarkdown files, put the script into the inst > rmd directory of the package and give *renderReport()* the new filename.
```
...
*plot0r* contains a numbe rof ggplot functions to generate plots from galvanostatic cycling data based on the processed sample data.
Below you find a brief overview:
- *plotGC()* plots of galvanostatic cycling (GC) experiments (Q vs. CycNr & CE vs. CycNr)
...
- *plotdQdV()* plots differential capacity plots (dQ/dV) - uses a moving average to smooth the data!
    return(p.dQdV)
    
- *plotdVdQ()* plots differential voltage plots (dV/dQ) - uses a moving average to smooth the data!
    return(p.dVdQ)

Either of the functions return one or several plots from a single sample list.
The same plots are used to generate the RMarkdown HTML report.
...
The style of the figures can be customized by theme functions. This package contains two themes: *theme1()* and *theme2()*, with *theme2()* being the default theme for all plots. Changing the theme can be done straightforwardly:
...
The figure style can also changed by custome themes in the same way, using local scripts (outside the package). 
...
> In *report0r()* or *process0r()* the data analysis might be stopped due to the error message below. This error is caused by wrong assignment of the potentiostat in the metadata file. Due to different column labelling, *select()* (dplyr package) cannot find the designated potential column.

Solution: edit the metadata file, i.e. update the cycler information or rename the potential column to "Ewe/V" and use "Biologic VMP" in the metadata file.
...
> In *report0r(htmlReport = TRUE)* or *renderReport()* RMarkdown may through the following error, if the filename contains special characters, especially " % "
