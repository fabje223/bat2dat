...
### Arbin Data
Arbin data comes in 2 formats: it is saved by the instrument as a *.res* file and can be converted via the Excel plug-in to a *.xlsx* result file with several tabs for metadata. raw cycling data, (processed) Stats-Tab for end-of-cycling-sequence information (e.g. cycle number vs. capacity). Both types can be imported into R. Import of *.res* files require the RODBC package (included in the package recommendations above) and changing the file extension from .res to *.accdb* (Access database file).
...
>   - manually: cylces = c(1,6,9,3,12,99) --> no specific order needed, just type the numbers you'd like to extract\
